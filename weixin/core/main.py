...
# 确保 weixin/ 根目录在 sys.path 中，支持从 core/ 直接运行
        ...
        async with ClientSession(read, write) as session:
            ...
            try:
                ...
                for index in (range(1, max_videos + 1) if max_videos > 0 else itertools.count(1)):
                    print(f"\n{'=' * 50}\n[video] index={index} {'(full collection mode)' if max_videos <= 0 else ''}")
                ...
                if collected_records:
                    print(f"\n{'=' * 60}")
                    print(f"[batch-asr] 开始批量 ASR，共 {len(collected_records)} 条视频")
                    print(f"{'=' * 60}")
                    from asr.sensevoice import run_asr_pipeline
                    for idx, rec in enumerate(collected_records, 1):
                        wav = rec.media_info.get("recording_audio_path")
                        if not wav:
                            print(f"[batch-asr] [{idx}/{len(collected_records)}] 无音频，跳过")
                            continue
                        if rec.media_info.get("asr_text"):
                            print(f"[batch-asr] [{idx}/{len(collected_records)}] 已有 ASR，跳过")
                            continue
                        vid = rec.candidate.get("video_identifier", "")
                        print(f"[batch-asr] [{idx}/{len(collected_records)}] vid={vid[:16]}...")
                        try:
                            run_asr_pipeline(rec, str(wav), vid)
                            # 重新保存 JSON + HTML（更新后的 ASR 和剧本比对结果）
                            from store import save_record
                            save_record(rec)
                            print(f"[batch-asr] [{idx}/{len(collected_records)}] 已更新")
                        except Exception as e:
                            print(f"[batch-asr] [{idx}/{len(collected_records)}] ASR 失败: {e}")
                    print(f"[batch-asr] 批量处理完成")
